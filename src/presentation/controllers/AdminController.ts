...
class AdminController {
  public async Autenticar(Request: Request, Response: Response) {
    try {
      ...
      const Administrador: any =
        await new PrismaClient().administrador.findFirst({ where: { email } });

      if (!Administrador)
        return Response.status(401).send({
          mensagem: "Administrador Inexistente",
        });
  ...
  public async ListarTodosAdministradores(
    Request: Request,
    Response: Response
  ) {
    try {
      const Administradores: any =
        await new PrismaClient().administrador.findMany();

      Administradores.map((administrador: any) => {
        return delete administrador.senha;
      });
  ...
  public async ListarAdmin(Request: Request, Response: Response) {
    try {
      const { id } = Request.params;

      const Administrador: any =
        await new PrismaClient().administrador.findUnique({ where: { id } });
