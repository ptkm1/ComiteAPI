...
class RelatoriosController {
  public async ProdutividadeDiária(Request: Request, Response: Response) {
    const { Status, DataSolicitacaoInicial, DataSolicitacaoFinal, Orgao } =
      Request.body;

    try {
      const Data = await new PrismaClient()
        .$queryRaw`SELECT * from RG WHERE Status 
  ...
  public async Reeimpressao(Request: Request, Response: Response) {
    const {
      Status,
      DataDeSolicitacaoInicial,
      DataDeSolicitacaoFinal,
      NomeCompleto,
      NomeDaMae,
    } = Request.body;
    const data = Request.body;

    // console.log(DataSolicitacaoInicial);
    console.log(DataDeSolicitacaoFinal);

    try {
      const Data = await new PrismaClient()
        .$queryRaw`SELECT * from RG WHERE Status like ${Status} and DataDeSolicitacao between ${DataDeSolicitacaoInicial} and ${DataDeSolicitacaoFinal} order by DataDeSolicitacao, NomeCompleto asc`;
      console.log(Data);
      Response.status(200).send(Data);
    } catch (error) {
      console.log(error);
  ...
  public async Agendamento(Request: Request, Response: Response) {
    const {
      DataDeAgendamentoInicial,
      DataDeAgendamentoFinal,
      LocalDeAgendamento,
    } = Request.body;
